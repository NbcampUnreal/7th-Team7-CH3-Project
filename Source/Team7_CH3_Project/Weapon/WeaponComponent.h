...
private:	
	...
	bool CanFire() const;                                                   // 사격 가능 조건 체크
	FVector GetFireDirection(const FVector& StartPos) const;                // 사격 방향 계산
	void ProcessHit(const FHitResult& Hit);                                 // 피격 처리 (데미지, 효과)
	void PlayFireEffects(const FVector& StartPos, const FName& SocketName); // 발사 피드백
